...
- **Joint training from warm start >> from scratch.** obj-018 showed joint training from random init hurts (underfitting). But obj-022 shows joint fine-tuning from a warm-started Receiver closes 93% of the accent gap (1.10× oracle vs 2.43× emitter-only). The key difference: warm start gives joint optimization a good basin to refine from, while random init requires discovering encoding+decoding simultaneously. When adapting to a new channel, always warm-start the Receiver from the previous channel, then jointly fine-tune.
